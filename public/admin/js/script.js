...
if (tablePermissions) {
    ...
    if (buttonSubmit) {
        buttonSubmit.addEventListener("click", () => {
            ...
            if (listElementRoleId) {
                listElementRoleId.forEach(elementRoleId => {
                    const id = elementRoleId.getAttribute("role-id");

                    const permissions = [];

                    const listInputChecked = document.querySelectorAll(`input[data-id="${id}"]:checked`);
            ...
            const buttonPatch = document.querySelector("button[data-path]");
            const path = buttonPatch.getAttribute("data-path");
